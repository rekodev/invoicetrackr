...
type Props = {
  ...
  >;
  existingBankAccount?: BankingInformationFormModel;
};
...
export default function BankingInformationSelect({
  ...
  const currentBankAccountId = bankAccounts?.find(
    (acc) =>
      acc.accountNumber === existingBankAccount?.accountNumber &&
      acc.code === existingBankAccount.code &&
      acc.name === existingBankAccount.name,
  )?.id;

  useEffect(() => {
    if (!existingBankAccount?.id || !user || !currentBankAccountId) return;

    setValue(new Set([currentBankAccountId.toString()]));
  }, [currentBankAccountId, user, existingBankAccount?.id, bankAccounts]);
